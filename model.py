...
class denoiser(object):
    ...
    def evaluate(self, iter_num, eval_files, noisy_files, summary_writer):
        ...
        for i in range(10):
            clean_image = cv2.imread(eval_files[i]) #index error
    ...
    def train(self, eval_files, noisy_files, batch_size, ckpt_dir, epoch, lr, eval_every_epoch=1):
        ...
        for epoch in range(start_epoch, epoch):
            ...
            for batch_id in range(start_step, numBatch):
              ...
              print("------------")
              print("res.shape[0]/2: ", res.shape[0]/2)
              print("------------")

              ind1 = range(int(res.shape[0]/2)) #Fixed Error: TypeError: 'float' object cannot be interpreted as an integer
...
#                cv2.imshow('noisy',batch_noisy[i])
    ...
    def test(self, eval_files, noisy_files, ckpt_dir, save_dir, temporal):
        ...
        print("------------------------------")
        print("len of eval_files: ", len(eval_files))
        print("eval_files = ", eval_files)
        print("------------------------------")

        assert len(eval_files) != 0, 'No testing data!'
        load_model_status, global_step = self.load(ckpt_dir)
        #assert load_model_status == True, '[!] Load weights FAILED...' #Error
